...
public class Hero_Ctrl : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    //데미지 연출
    Shader m_DefShader;
    Shader m_WeaponShader;

    bool IsAtttackColorChange = false;
    SkinnedMeshRenderer m_SMR = null;
    SkinnedMeshRenderer[] m_SMRList = null;
    MeshRenderer[] m_MeshList = null;
    float AttachColorStartTime = 0.0f;
    float AttachColorTime = 0.1f;
    float m_Ratio = 0.0f;
    float m_fcol = 0;
    ...
    {
        //자기가 조종하고 있는 Player 기분으로 처리해 준다.
        if (pv.IsMine == false)
            return;
    ...
    void FindDefShade()
    {
        if (m_SMR == null)
        {
            ...
            if (m_SMR != null)
                m_DefShader = m_SMR.material.shader;

            if (0 < m_MeshList.Length)
                m_WeaponShader = m_MeshList[0].material.shader;
    ...
    void AttachColorUpdate()
    {
        ...
        if (IsAtttackColorChange == false) return;

        FindDefShade();

        m_Ratio = (Time.time - AttachColorStartTime) / AttachColorTime;
        m_Ratio = Mathf.Min(1.0f, m_Ratio);
        ...
        if (1 <= m_Ratio)
        {
            ...
            {
                if (m_DefShader != null)
                    m_SMRList[i].material.shader = m_DefShader;
            }
            ...
            if (m_MeshList != null)
            {
                ...
                {
                    if (m_WeaponShader != null)
                        m_MeshList[i].material.shader = m_WeaponShader;
                }

            }

            IsAtttackColorChange = false;
        }
        else
        {
            for (int i = 0; i < m_SMRList.Length; i++)
