...
public class MonsterCtrl : MonoBehaviourPunCallbacks, IPunObservable
{
    ...
    #region Die 연출
    Vector3 m_DieDir = Vector3.zero; //죽는 방향 벡터
    float m_DieDur = 0.0f;         //죽는 속도
    float m_DieTime = 0.0f;        //죽는 시간

    SkinnedMeshRenderer[] m_SMR;
    SkinnedMeshRenderer[] m_SMRList;
    MeshRenderer[] m_MeshList;
    float m_Ratio = 0.0f;
    Color m_CalcColor;
    ...
    public void TakeDamage(GameObject a_Attacker, float a_Damage = 10.0f)
    {
        ...
        if (pv.IsMine == true) //사망 처리도 IsMine 일때만... 나머지는 동기화로 처리함
            if (CurHp <= 0.0f)   //사망처리
            {
                ...
                ChangeAnimState(AnimState.die, 0.1f);
                FindShader();
                StartCoroutine("DieDir");
            }
    }//public void TakeDamage(GameObject a_Attacker, float a_Damage = 10.0f)
    ...
    void CreateItem()
    {
        ...
        //int a_Rand = Random.Range(0, 11);
        //if (2 < a_Rand) return;

        Vector3 a_HPos = transform.position;


        PhotonNetwork.InstantiateRoomObject("DiamondItem/DiamondPrefab",
                                                    a_HPos, Quaternion.identity, 0);
    ...
    void FindShader()
    {
        if (m_SMR == null)
        {
            m_SMR = GetComponentsInChildren<SkinnedMeshRenderer>();
    ...
    IEnumerator DieDir()
    {
        ...
        BoxCollider a_Box = gameObject.GetComponentInChildren<BoxCollider>();
        ...
        while (true)
        {
            m_Ratio = m_DieTime / m_DieDur;
            m_Ratio = Mathf.Min(m_Ratio, 1);
            m_CalcColor = new Color(1, 1, 1, m_Ratio);

            if (0.9f < m_Ratio && 0 < m_DieDir.magnitude)
            {
                transform.position= transform.position + m_DieDir * (((m_Ratio * 0.38f) * 14f) * Time.deltaTime);
            }

            if (m_Ratio < 0.83f)
            {
                if (m_Canvas == null) m_Canvas = transform.Find("Canvas");
            ...
            //monster shader change
            for (int i = 0; i < m_SMRList.Length; i++)
            {
                if (GameMgr.Inst.g_VertextShader != null && m_SMRList[i].material.shader != GameMgr.Inst.g_VertextShader)
                {
                    m_SMRList[i].material.shader = GameMgr.Inst.g_VertextShader;
                }
            ...
            //weapon shader change
            if (m_MeshList != null)
            {
                for (int i = 0; i < m_MeshList.Length; i++)
                {
                    if (GameMgr.Inst.g_VertextShader != null && m_MeshList[i].material.shader != GameMgr.Inst.g_VertextShader)
                        m_MeshList[i].material.shader = GameMgr.Inst.g_VertextShader;
            ...
            if (m_DieTime < 0)
            {
                if (pv.IsMine == true)
                {
                    MonSpawn_Mgr.Inst.ResetTime(m_SpawnIdx);
