...
public class PhotonInit : MonoBehaviourPunCallbacks  //MonoBehaviour
{
    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList)
    {
        ...
        for(int i = 0; i < roomCount; i++)
        {
            ...
            { //방을 새로 생성하거나, 방정보를 갱신해 줘야 하는 상황
                if(a_ArrIdx < 0)
                {  //방을 새로 생성하거나, 방정보를 갱신해 줘야 하는 상황
                    // 스크롤 뷰에 붙여줄 새로운 방 오브젝트를 새로 생성해 줘야 함
                    //--- 새로운 방 오브젝트 새로 생성
                    GameObject room = Instantiate(roomItem) as GameObject; 
                    //생성한 RoomItem 프리팹의 Parent를 저장
                ...
                {  //해당 방이 리스트 뷰에 존재하면 방정보만 갱신해 줘야 한다는 의미
                    //기본 방 정보만 갱신
            ...
            { //방이 파괴되어서 리스트뷰에서 RoomItem을 제거해 줘야 하는 상황
                if(0 <= a_ArrIdx)
                {
